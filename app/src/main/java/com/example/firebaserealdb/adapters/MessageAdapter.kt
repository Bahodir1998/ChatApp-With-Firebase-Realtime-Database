...
import com.example.firebaserealdb.databinding.ToItemBinding
...
class MessageAdapter(var list:List<Message>):RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class FromVh(var fromItemBinding: FromItemBinding):RecyclerView.ViewHolder(fromItemBinding.root){
        fun onBind(message: Message){
            fromItemBinding.messageTv.setText(message.message)
            fromItemBinding.date.setText(message.date?.substring(11))
        }
    }

    inner class ToVh(var toItemBinding: ToItemBinding):RecyclerView.ViewHolder(toItemBinding.root){
        fun onBind(message: Message){
            toItemBinding.messageTv.setText(message.message)
            toItemBinding.date.setText(message.date?.substring(11))
        }
    }
